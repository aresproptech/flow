...
begin
  ...
  where id = p_opportunity_id and deleted_at is null for update;
  if not found then raise exception 'El lead no existe o está eliminado' using errcode = 'P0002'; end if;
  ...
  if not exists (
    select 1 from storage.objects where bucket_id = 'lead-documentation' and name = storage_path
  ) then raise exception 'El archivo no existe en Storage' using errcode = 'P0002'; end if;

  insert into public.opportunity_documentation_files (
    opportunity_id, requirement_key, file_name, storage_path, mime_type, file_size, uploaded_by
  ) values (
    p_opportunity_id, requirement_key, file_name, storage_path, mime_type, file_size, actor_name
  ) returning * into saved_file;

  activity_text := 'Subió el documento «' || file_name || '»';
  insert into public.opportunity_activities (
    opportunity_id, fecha, memo, resultado, event_type, actor_profile_id, effective_at, metadata
  ) values (
    p_opportunity_id, current_date, null, true, 'document_uploaded', actor_id, now(),
    jsonb_build_object(
      'actor_name', actor_name, 'text', activity_text, 'document_id', saved_file.id,
      'requirement_key', requirement_key, 'file_name', file_name, 'storage_path', storage_path,
...
begin
  ...
  select * into target_file from public.opportunity_documentation_files where id = p_file_id;
  ...
  if not exists (
    ...
    where bucket_id = 'lead-documentation' and name = target_file.storage_path
  ) then raise exception 'El archivo no existe en Storage' using errcode = 'P0002'; end if;
  ...
  insert into public.opportunity_activities (
    ...
    target_file.opportunity_id, current_date, null, true, 'document_viewed', actor_id, now(),
    jsonb_build_object(
      'actor_name', actor_name, 'text', activity_text, 'document_id', target_file.id,
      'requirement_key', target_file.requirement_key, 'file_name', target_file.file_name,
